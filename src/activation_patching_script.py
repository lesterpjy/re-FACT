...
def create_patches(model, all_names_df, df2, batch_size, needed_tokens, directory, dont_duplicate: bool = True):
    """
    Creates activation patches and saves them in the directory
    :param prompts2: list of prompts
    :param model: model
    :param all_names_dict: dictionary of names with key as the length of the name
    ...
    for i in range(0, len(prompts2), batch_size):
        ...
        # corrupted_prompts, _ = corrupt_prompt_names(current_prompts, df2.loc[i:i+batch_size-1, 'name'].to_list(), model, df2.loc[i:i+batch_size-1, 'Sub_Region'].to_list(), all_names_df)
...
def main(args):
    ...
    wiki_all_names = pd.read_csv(args.wiki_names_path)
    ...
    # sentences2 = df["new_sentence"].to_list()
    # prompts2 = [PROMPT_TEMPLATE.format(sentence) for sentence in sentences2]
    ...
    if args.calculate_patches:
        create_patches(model, all_names_df, df, batch_size, needed_tokens, dir_name, dont_duplicate=True)
    ...
    # all_clean_logits = create_clean_logits_from_dir(dir_name)
    # baseline_logit_diff = compute_logit_diff_2(all_clean_logits, all_clean_tokens, df.loc[:, 'toxic'].to_list(), needed_tokens, average=True).to("cuda")
    # torch.save(baseline_logit_diff, os.path.join(dir_name, "baseline_logit_diff.pth"))
